...
2. **Enumerated deliverables, each with a completeness definition.** Letter every section (a, b, c...) and define what "complete" means inside it ("every command — flags short+long, defaults, examples for both human and `--json`/script usage"). The agent fills the frame you give; no frame → agent picks its own depth. The enumeration determines the document's shape — for common spec types (CLI surface, agent/skill behavior), start from the proven menus in `references/deliverable-menus.md` instead of inventing the list; spec deliverables exclude stack/structure/code (those go to the plan).
