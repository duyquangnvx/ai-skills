...
The mixed cut runs through a single artifact as readily as across several. A forty-term glossary of which four are used every turn is two items, not one: preload the four, leave the rest behind a lookup. Splitting by *what is actually reached for* is usually a larger saving than splitting by kind, and it applies to instruction files, schemas, and catalogs as much as to data.
...
"Point of use" is a scope, not a per-turn mandate, and the scope is **the unit of work bounded by the next consequential statement or write**. Reusing a value fetched two turns ago to keep discussing it is fine; carrying it across the boundary where the agent commits to something — an answer the user will act on, a write, an external message — is not. Set that boundary explicitly, because leaving it implicit resolves to one of two failures: a re-fetch on every turn, or a snapshot that quietly lives as long as the session.
