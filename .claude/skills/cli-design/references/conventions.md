...
| `DO_NOT_TRACK` | Cross-tool telemetry opt-out — honor it (gh CLI is the model) |
...
XDG paths (respect the env vars; fall back to defaults when unset or empty):

| Purpose | Env var | Default |
|---|---|---|
| Config | `XDG_CONFIG_HOME` | `~/.config/myapp` |
| Data | `XDG_DATA_HOME` | `~/.local/share/myapp` |
| State (history, logs) | `XDG_STATE_HOME` | `~/.local/state/myapp` |
| Cache | `XDG_CACHE_HOME` | `~/.cache/myapp` (macOS: `~/Library/Caches/myapp`; Windows: `%LOCALAPPDATA%\myapp`) |

`XDG_*_HOME` (user) takes precedence over `XDG_*_DIRS` (system search paths).
...
Off by default or clearly disclosed opt-out at minimum; opt-in preferred. Disclose what, why, anonymization, retention. Honor the cross-tool `DO_NOT_TRACK` convention AND provide a tool-specific variable (gh CLI model: `GH_TELEMETRY` takes precedence over `DO_NOT_TRACK`). Trivial to disable, non-interactively, for CI.
