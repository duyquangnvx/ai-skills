...
- **Set `process.exitCode` instead of calling `process.exit()`.** Node documents that `process.exit()` forces termination even with writes to stdout/stderr still pending — output gets truncated, typically exactly when piping to a slower consumer. Set the code, let the event loop drain, and reserve `process.exit()` for genuine must-die-now cases.
...
- Target a modern Node version with `platform: node`, and declare the floor in package.json `engines`. The stdlib APIs this skill leans on land at specific versions: `util.parseArgs` stable since **20**, `util.styleText` stable since **22.13**, `--env-file`/`process.loadEnvFile` stable in the **24** line. A floor of **Node ≥ 22** is the pragmatic default today; guard or polyfill anything newer than your floor.
...
| `process.exit(1)` at the boundary | `process.exitCode = 1` — force-exit truncates pending stdout/stderr writes. |
