...
3. **Load the relevant reference file(s)** for concrete rules and examples:
   ...
   - `references/architecture.md` — thin-command/fat-core layering, the canonical project structure (`cli.ts` + `commands/` + `core/` + `adapters/` + `ui/` + `lib/`), enforcing boundaries with tooling (dependency-cruiser/eslint-plugin-boundaries), layer rules (ports & adapters), cross-cutting base command/error boundary, bootstrap & init order, command naming/discoverability, startup performance (fast-paths, lazy command modules, build-time gates), type safety (strict + Zod at the boundary), CLI behavior contract, build & distribution (bin/shebang, esbuild bundling, ESM, build-time macros), testing strategy, common mistakes. **Read this for any task about how the code is organized in TypeScript/Node, or about keeping a growing CLI testable.**
