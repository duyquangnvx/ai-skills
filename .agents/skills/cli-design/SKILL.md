...
3. **Load the relevant reference file(s)** for concrete rules and examples:
   ...
   - `references/architecture.md` — thin-command/fat-core layering, the single canonical project structure (a thin `cli/` shell over feature-first slices) and its scale-invariant growth rule (grow a feature file into a folder, never re-slice), layer rules (ports & adapters), cross-cutting base command/error boundary, bootstrap & init order, command naming/discoverability, scaling to a monorepo + plugins, startup performance (fast-paths, lazy loading, build-time gates), type safety (strict + Zod at the boundary), CLI behavior contract, build & distribution (bin/shebang, esbuild bundling, ESM, build-time macros), testing strategy, common mistakes. **Read this for any task about how the code is organized in TypeScript/Node, or about keeping a growing CLI testable.**
