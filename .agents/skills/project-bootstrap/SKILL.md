...
Up to five files in `docs/plans/`. Four map to different time orientations, which is why each exists separately; the fifth is the folder index. `architecture.md` and `decisions.md` are optional — if the project already maintains an equivalent doc elsewhere, link to it from `README.md` instead of creating a duplicate here. Judge equivalence by role at this layer, not by filename: a dev-layer `architecture.md` (system components, data flow) is NOT a duplicate of the product-shape doc here, and a dev-layer decisions log (library picks, patterns) is NOT a duplicate of the product-level decisions log here — when both layers exist, cross-link them rather than merging. The unique contribution of this skill is the forward view (`roadmap.md`) and the live status (`progression.md`).
...
4. **`progression.md`** — Use `assets/progression.md.template`. This file tracks **phase-level** status. If the repo keeps a session-level progress snapshot (e.g., a root `progress.md`), the two coexist at different cadences — add a link, do not merge them. Initial state: phase 1 not started, no blockers, "Next" pointing to phase 1 kickoff. Current focus section filled with "Active work: not yet begun" and "Immediate next steps: kick off Phase 1." The Log section starts with one entry: "<date> — planning artifacts initialized."
